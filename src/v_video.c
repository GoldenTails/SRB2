...
void V_SetPalette(INT32 palettenum)
{
	...
	if (rendermode != render_none)
		SWR_SetPalette(&pLocalPalette[palettenum*256]);
}
...
#endif
		SWR_DrawStretchyFixedPatch(x, y, pscale, vscale, scrn, patch, colormap);
}
...
#endif
		SWR_DrawCroppedPatch(x, y, pscale, vscale, scrn, patch, colormap, sx, sy, w, h);
}
...
{
	SWR_DrawBlock(x, y, scrn, width, height, src);
}
...
#endif
		SWR_DrawPic(rx1, ry1, scrn, lumpnum);
}
...
#endif
		SWR_DrawFadeFill(x, y, w, h, c, color, strength);
}
...
#endif
		SWR_DrawFlatFill(x, y, w, h, flatnum);
}
...
#endif
		SWR_DrawFadeScreen(color, strength);
}
...
#endif
		SWR_DrawPromptBack(color, boxheight);
}
...
void V_DoPostProcessor(INT32 view, postimg_t type, INT32 param)
{
	...
	SWR_DoPostProcessor(view, type, param);
}
