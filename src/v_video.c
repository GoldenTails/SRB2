...
void V_DrawUnicodeChatCharacter(INT32 x, INT32 y, char *utf8, INT32 flags, boolean lowercaseallowed, UINT8 *colormap)
{
	...
	w = (vid.width < 640 ) ? (chr->width/2) : (chr->width);	// use normal sized characters if we're using a terribly low resolution.
	if (x + w > vid.width)
		return;
...
static void V_DrawNameTagLine(INT32 x, INT32 y, INT32 option, fixed_t scale, UINT8 *basecolormap, UINT8 *outlinecolormap, const char *string)
{
	...
	for (;;)
	{
		...
		if (M_CharIsUnicode(c))
		{
			...
			ys = FixedMul(scale, FixedDiv(20*FRACUNIT, (pat->height * FRACUNIT)));
			w = FixedMul(w, xs);
		...
		if (!isUTF8)
		{
			...
			w = FixedMul(((ntb_font[c]->width)+2 * dupx) * FRACUNIT, xs);
		}
...
INT32 V_StringWidth(const char *string, INT32 option)
{
	...
	for (i = 0; i < strlen(string);)
	{
		...
		if (!pat)
			...
			w += (charwidth ? charwidth : pat->width);
...
INT32 V_SmallStringWidth(const char *string, INT32 option)
{
	...
	for (i = 0; i < strlen(string);)
	{
		...
		if (!pat)
			...
			w += (charwidth ? charwidth : pat->width/2);
