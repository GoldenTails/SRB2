...
static boolean SWR_IsFFloorTranslucent(visffloor_t *pfloor)
{
	if (pfloor->polyobj)
...
static boolean SWR_FFloorCanClip(visffloor_t *pfloor)
{
	return (cv_ffloorclip.value && !SWR_IsFFloorTranslucent(pfloor) && !pfloor->polyobj);
}
				...
				if (ffloor[i].height < viewz)
				{
					...
					else
					{
						if (top_w <= bottom_w)
						{
							...
							if (ffbottom >= bottom-1 && SWR_FFloorCanClip(&ffloor[i]) && !curline->polyseg)
							{
								rw_floormarked = true;
				...
				else if (ffloor[i].height > viewz)
				{
					...
					else
					{
						if (top_w <= bottom_w)
						{
							...
							if (fftop <= top+1 && SWR_FFloorCanClip(&ffloor[i]) && !curline->polyseg)
							{
